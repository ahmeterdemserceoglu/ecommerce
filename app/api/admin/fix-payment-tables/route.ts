...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sessionError) {
      console.error("[API /api/admin/fix-payment-tables POST] Error getting session:", sessionError.message)
      return NextResponse.json({ error: "Session error: " + sessionError.message }, { status: 500 })
    }
    if (!session) {
      console.log("[API /api/admin/fix-payment-tables POST] No session found.")
    ...
    const { data: userProfile, error: profileError } = await supabaseAuth
      ...
      .single()
    if (profileError) {
      console.error(`[API /api/admin/fix-payment-tables POST] Error fetching profile for user ${session.user.id}:`, profileError.message)
      return NextResponse.json({ error: "Failed to fetch user profile for authorization." }, { status: 500 })
    }
    if (!userProfile || userProfile.role !== 'admin') {
      console.warn(`[API /api/admin/fix-payment-tables POST] Authorization failed. User role: ${userProfile?.role} (Expected 'admin')`)
      return NextResponse.json({ error: "Unauthorized: Admin access required." }, { status: 403 })
    }
    console.log(`[API /api/admin/fix-payment-tables POST] Admin user ${session.user.id} authorized.`)
